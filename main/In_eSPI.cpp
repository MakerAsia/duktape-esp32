...
void TFT_eSPI::DC_C() {
    GPIO.out_w1ts = (1 << PIN_TFT_DC);
    GPIO.out_w1tc = (1 << PIN_TFT_DC);
} 
...
void TFT_eSPI::CS_L() {
  GPIO.out_w1ts = (1 << PIN_TFT_CS);
  GPIO.out_w1tc = (1 << PIN_TFT_CS);
} 
...
if( PIN_TFT_CS >= 0 ) {
  digitalWrite(PIN_TFT_CS, HIGH); // Chip select high (inactive)
...
if( PIN_TFT_DC >= 0 ) {
  digitalWrite(PIN_TFT_DC, HIGH); // Data/Command high = data mode
  pinMode(PIN_TFT_DC, OUTPUT);
}

if (PIN_TFT_RST >= 0) {
  digitalWrite(PIN_TFT_RST, HIGH); // Set high, do not share pin with another SPI device
...
void TFT_eSPI::spi_init(void)
{
	SPI.end();
#if !defined (ESP32)
  if( PIN_TFT_CS >= 0 ) {
    cspinmask = (uint32_t) digitalPinToBitMask(PIN_TFT_CS);
  }

 if( PIN_TFT_DC >= 0 ) {
    dcpinmask = (uint32_t) digitalPinToBitMask(PIN_TFT_DC);
 }
...
#if defined(ESP32_PARALLEL)
    digitalWrite(PIN_TFT_CS, LOW); // Chip select low permanently
    pinMode(PIN_TFT_CS, OUTPUT);
#else
  if( PIN_TFT_CS >= 0 ) {
...
int16_t TFT_eSPI::textWidth(const char *string, int font)
{
  ...
  else
  {
    ...
    if(gfxFont) // New font
    {
      ...
      {
        uniCode = *(string++);
        if ((uniCode >= (uint8_t)pgm_read_word(&gfxFont->first)) && (uniCode <= (uint8_t)pgm_read_word(&gfxFont->last )))
        {
...
int16_t TFT_eSPI::fontHeight(int16_t font)
{
  ...
  if (font==1)
  {
    ...
    {
      if( printUTF8 ) {
        return glyph_ab * textsize + glyph_bb * textsize;
      }
...
***************************************************************************************/
void TFT_eSPI::drawChar(int32_t x, int32_t y, unsigned int c, uint32_t color, uint32_t bg, uint8_t size)
{
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>
    ...
    if ((c >= (uint16_t)pgm_read_word(&gfxFont->first)) && (c <= (uint16_t)pgm_read_word(&gfxFont->last )))
    {
      spi_begin();
      inTransaction = true;
//>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  ...
  else
  {
    ...
    if(utf8 == '\n') {
      ...
      if (uniCode > (uint8_t)pgm_read_word(&gfxFont->last )) return 0;
      if (uniCode < (uint8_t)pgm_read_word(&gfxFont->first)) return 0;
...
size_t TFT_eSPI::write(const uint8_t *s, size_t size)
{
    size_t n = 0;
    unsigned int wc;
    if( printUTF8 ) {
        ...
        while( (c = s[i]) !=0 ) {
            ...
            if( wc == 13 ) {
                ...
                //cursor_y += pgm_read_byte(&gfxFont->yAdvance) * textsize;
                cursor_y += glyph_ab * textsize + glyph_bb * textsize;
            }
            else {
                cursor_x += drawChar( wc, cursor_x, cursor_y, 1);
                n++;
            }
        }
...
void TFT_eSPI::setFreeFont(const GFXfont *f)
{
  ...
  glyph_ab = 0;
  glyph_bb = 0;
  uint8_t numChars = pgm_read_word(&gfxFont->last) - pgm_read_word(&gfxFont->first);
...
if (PIN_TFT_MOSI >= 0 )
  tft_settings.pin_tft_mosi = PIN_TFT_MOSI;
else
  tft_settings.pin_tft_mosi = -1;

if( PIN_TFT_MISO >= 0 )
  tft_settings.pin_tft_miso = PIN_TFT_MISO;
else
  tft_settings.pin_tft_miso = -1;

if (PIN_TFT_SCLK >= 0)
  tft_settings.pin_tft_clk  = PIN_TFT_SCLK;
else
  tft_settings.pin_tft_clk  = -1;

if (PIN_TFT_CS >= 0)
  tft_settings.pin_tft_cs   = PIN_TFT_CS;
else
  tft_settings.pin_tft_cs   = -1;

if (PIN_TFT_DC >= 0 )
  tft_settings.pin_tft_dc  = PIN_TFT_DC;
else
...
if (PIN_TFT_RST >= 0)
  tft_settings.pin_tft_rst = PIN_TFT_RST;
else
