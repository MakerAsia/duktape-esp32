...
class TFT_eSPI : public Print {
 ...
 public:
  ...
  int PIN_TFT_MOSI=-1;
  int PIN_TFT_SCLK=-1;
  int PIN_TFT_CS=-1;  // Chip select control pin
  int PIN_TFT_DC=-1;  // Data Command control pin  
  int PIN_TFT_BL=-1;  // Data Command control pin  
  int PIN_TFT_RST=-1;  // Data Command control pin  
  int TFT_DEVICE_M5STACK = false;
  ...
  virtual void     drawPixel(uint32_t x, uint32_t y, uint32_t color),
                   drawChar(int32_t x, int32_t y, unsigned int c, uint32_t color, uint32_t bg, uint8_t size),
