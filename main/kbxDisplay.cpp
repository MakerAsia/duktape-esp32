...
kbxDisplay::kbxDisplay(int16_t w, int16_t h) : TFT_eSPI(w,h) {}

void kbxDisplay::begin() {
  TFT_eSPI::begin();
  setRotation(1);
  fillScreen(0);
...
void kbxDisplay::sleep() {
  startWrite();
  writecommand(ILI9341_SLPIN); // Software reset
  endWrite();
}

void kbxDisplay::setBrightness(uint8_t brightness) {
  if( PIN_TFT_BL >= 0 )
    ledcWrite(BLK_PWM_CHANNEL, brightness);
}

void kbxDisplay::drawBitmap(int16_t x0, int16_t y0, int16_t w, int16_t h, const uint16_t *data) {
  setSwapBytes(true);
  pushImage((int32_t)x0, (int32_t)y0, (uint32_t)w, (uint32_t)h, data);
}

void kbxDisplay::drawBitmap(int16_t x0, int16_t y0, int16_t w, int16_t h, uint16_t *data) {
  setSwapBytes(true);
  pushImage((int32_t)x0, (int32_t)y0, (uint32_t)w, (uint32_t)h, data);
}

void kbxDisplay::drawBitmap(int16_t x0, int16_t y0, int16_t w, int16_t h, const uint16_t *data, uint16_t transparent) {
  setSwapBytes(true);
  pushImage((int32_t)x0, (int32_t)y0, (uint32_t)w, (uint32_t)h, data, transparent);
}

void kbxDisplay::drawBitmap(int16_t x0, int16_t y0, int16_t w, int16_t h, const uint8_t *data) {
  setSwapBytes(true);
  pushImage((int32_t)x0, (int32_t)y0, (uint32_t)w, (uint32_t)h, (const uint16_t*)data);
}

void kbxDisplay::drawBitmap(int16_t x0, int16_t y0, int16_t w, int16_t h, uint8_t *data) {
  setSwapBytes(true);
  pushImage((int32_t)x0, (int32_t)y0, (uint32_t)w, (uint32_t)h, (uint16_t*)data);
}

void kbxDisplay::progressBar(int x, int y, int w, int h, uint8_t val) {
  drawRect(x, y, w, h, 0x09F1);
  fillRect(x + 1, y + 1, w * (((float)val) / 100.0), h - 1, 0x09F1);
}

#include "qrcode.h"
void kbxDisplay::qrcode(const char *string, uint16_t x, uint16_t y, uint8_t width, uint8_t version) {
...
void kbxDisplay::qrcode(const String &string, uint16_t x, uint16_t y, uint8_t width, uint8_t version) {
...
void kbxDisplay::drawBmpFile(fs::FS &fs, const char *path, uint16_t x, uint16_t y) {
...
// void kbxDisplay::drawBmp(fs::FS &fs, const char *path, uint16_t x, uint16_t y) {
...
void kbxDisplay::drawJpg(const uint8_t *jpg_data, size_t jpg_len, uint16_t x,
...
void kbxDisplay::drawJpgFile(fs::FS &fs, const char *path, uint16_t x, uint16_t y,
...
void kbxDisplay::drawRGBBitmap(int16_t x, int16_t y,
